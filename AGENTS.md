...
- When presenting options, clearly separate verified facts from
  proposals.
- In plan mode, prefer exhaustive clarification over speculative
  implementation, and keep asking follow-up questions until the
  decision-making surface is explicit enough that the plan does not rely
  on hidden assumptions.
- When asking the user to choose, include the viable alternatives and
  the relevant trade-offs so the user can make an informed decision.
...
- State how subagents were used to reduce the main agent's context
  usage, or explain why meaningful delegation was not possible.
- Call out any missing information or assumptions that need user
  approval.
- For any plan-mode exchange, summarize the clarifying questions that
  were asked, the alternatives that were presented, and the trade-offs
